...
export default function SpeakConversation(){
  ...
  useEffect(()=>{
    ...
    void (async()=>{
      try{
        ...
        const conversation=await getLatestChatConversation()
        if(!active)return
        if(conversation){
  ...
  return <ScrollView contentContainerStyle={s.content} keyboardShouldPersistTaps="handled">
    <View style={s.header}><View style={s.logo}><Image source={require("../../assets/logo.png")} resizeMode="contain" style={s.logoImage}/></View><View><Text style={s.eyebrow}>AI CONVERSATION</Text><Text style={s.title}>Speak with Lingu</Text></View></View>
    <View style={s.hero}><View style={s.pill}><Ionicons name="sparkles" size={11} color="#C2BEFF"/><Text style={s.pillText}>YOUR LANGUAGE TUTOR</Text></View><Text style={s.heroTitle}>Practice naturally.</Text><Text style={s.heroBody}>Continue with Lingu using the authenticated JUBA LISAN tutor service. Your latest conversation is restored automatically when you return.</Text></View>
    {!authenticated?<View style={s.card}><View style={s.empty}><View style={s.tutor}><Ionicons name="arrow-forward" size={26} color="#fff"/></View><Text style={s.cardTitle}>Sign in to practice with Lingu</Text><Text style={s.body}>AI conversation requires an authenticated JUBA LISAN account. Sign in from the app to unlock the tutor.</Text></View></View>:loading?<View style={s.card}><View style={s.empty}><ActivityIndicator color={C.primary}/><Text style={s.body}>Loading your conversation…</Text></View></View>:<>
      <View style={s.toolbar}>
        <Text style={s.toolbarText}>{conversationId?`Conversation #${conversationId}`:'New conversation'}</Text>
      ...
      <View style={s.card}>
        {messages.length===0?<View style={s.empty}><View style={s.tutor}><Ionicons name="sparkles" size={28} color="#fff"/></View><Text style={s.cardTitle}>Tell me about your day</Text><Text style={s.body}>Write a short message and Lingu will answer. Start a new conversation anytime without losing your previous threads.</Text></View>:messages.map((m,i)=><View key={`${i}-${m.role}`} style={[s.message,m.role==='user'?s.userMessage:s.assistantMessage]}><Text style={[s.messageRole,m.role==='user'&&s.userRole]}>{m.role==='user'?'YOU':'LINGU'}</Text><Text style={s.messageText}>{m.content||'…'}</Text></View>)}
      </View>
      {!!memoryNotice&&<View style={s.memory}><Text style={s.memoryText}>✓ Lingu updated your learning memory from this conversation.</Text></View>}
