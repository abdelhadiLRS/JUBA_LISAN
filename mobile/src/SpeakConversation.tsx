...
export default function SpeakConversation({onRequestLogin}:{onRequestLogin:()=>void}){
  ...
  return <KeyboardAvoidingView style={s.keyboard} behavior={Platform.OS==='ios'?'padding':undefined}><ScrollView ref={scrollRef} contentContainerStyle={s.content} keyboardShouldPersistTaps="handled" keyboardDismissMode="on-drag">
    ...
    {!authenticated?<View style={s.card}><View style={s.empty}><View style={s.tutor}><Image source={require("../../assets/logo.png")} resizeMode="contain" style={s.tutorLogo}/></View><Text style={s.cardTitle}>Sign in to practice with your tutor</Text><Text style={s.body}>AI conversation requires an authenticated JUBA LISAN account. Sign in to unlock the tutor and keep your conversations synced.</Text><Pressable onPress={onRequestLogin} accessibilityRole="button" accessibilityLabel="Go to sign in" style={({pressed})=>[s.guestLoginButton,pressed&&s.pressed]}><Text style={s.guestLoginText}>Sign in</Text><Ionicons name="arrow-forward" size={16} color="#fff"/></Pressable></View></View>:loading?<View style={s.card}><View style={s.empty}><ActivityIndicator color={C.primary}/><Text style={s.body}>Loading your conversation…</Text></View></View>:<>
      ...
      <View style={s.card}>
        {messages.length===0?<View style={s.empty}><View style={s.tutor}><Image source={require("../../assets/logo.png")} resizeMode="contain" style={s.tutorLogo}/></View><Text style={s.cardTitle}>Tell me about your day</Text><Text style={s.body}>Write a short message and your tutor will answer. Start a new conversation anytime without losing your previous threads.</Text></View>:messages.map((m,i)=>m.role==='assistant'?<View key={`${i}-${m.role}`} style={s.assistantRow}><View style={s.messageAvatar}><Image source={require("../../assets/logo.png")} resizeMode="contain" style={s.messageAvatarImage}/></View><View style={[s.message,s.assistantMessage]}><Text style={s.messageRole}>TUTOR</Text><Text style={s.messageText}>{m.content||'…'}</Text></View></View>:<View key={`${i}-${m.role}`} style={[s.message,s.userMessage]}><Text style={[s.messageRole,s.userRole]}>YOU</Text><Text style={s.messageText}>{m.content||'…'}</Text></View>)}
      </View>
