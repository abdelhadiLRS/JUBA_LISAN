...
export default function SpeakConversation(){
  ...
  return <ScrollView contentContainerStyle={s.content} keyboardShouldPersistTaps="handled">
    ...
    <View style={s.hero}><Text style={s.pill}>✦ YOUR LANGUAGE TUTOR</Text><Text style={s.heroTitle}>Practice naturally.</Text><Text style={s.heroBody}>Continue with Lingu using the authenticated JUBA LISAN tutor service. Your latest conversation is restored automatically when you return.</Text></View>
    {!authenticated?<View style={s.card}><View style={s.empty}><Text style={s.tutor}>→</Text><Text style={s.cardTitle}>Sign in to practice with Lingu</Text><Text style={s.body}>AI conversation requires an authenticated JUBA LISAN account. Sign in from the app to unlock the tutor.</Text></View></View>:loading?<View style={s.card}><View style={s.empty}><ActivityIndicator color={C.primary}/><Text style={s.body}>Loading your conversation…</Text></View></View>:<>
      <View style={s.toolbar}>
        <Text style={s.toolbarText}>{conversationId?`Conversation #${conversationId}`:'New conversation'}</Text>
        <Pressable disabled={busy} onPress={()=>void startNewConversation()} style={s.newButton}><Text style={s.newButtonText}>New chat</Text></Pressable>
      </View>
      <View style={s.card}>
        {messages.length===0?<View style={s.empty}><Text style={s.tutor}>✦</Text><Text style={s.cardTitle}>Tell me about your day</Text><Text style={s.body}>Write a short message and Lingu will answer. Start a new conversation anytime without losing your previous threads.</Text></View>:messages.map((m,i)=><View key={`${i}-${m.role}`} style={[s.message,m.role==='user'?s.userMessage:s.assistantMessage]}><Text style={[s.messageRole,m.role==='user'&&s.userRole]}>{m.role==='user'?'YOU':'LINGU'}</Text><Text style={s.messageText}>{m.content||'…'}</Text></View>)}
      </View>
