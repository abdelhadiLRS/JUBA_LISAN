...
export default function SpeakConversation(){
  ...
  return <ScrollView contentContainerStyle={s.content} keyboardShouldPersistTaps="handled">
    ...
    {!authenticated?<View style={s.card}><View style={s.empty}><View style={s.tutor}><Ionicons name="arrow-forward" size={26} color="#fff"/></View><Text style={s.cardTitle}>Sign in to practice with Lingu</Text><Text style={s.body}>AI conversation requires an authenticated JUBA LISAN account. Sign in from the app to unlock the tutor.</Text></View></View>:loading?<View style={s.card}><View style={s.empty}><ActivityIndicator color={C.primary}/><Text style={s.body}>Loading your conversation…</Text></View></View>:<>
      ...
      {!!memoryNotice&&<View style={s.memory}><Ionicons name="checkmark-circle-outline" size={16} color={C.success}/><Text style={s.memoryText}>Lingu updated your learning memory from this conversation.</Text></View>}
