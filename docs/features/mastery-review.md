...
The dedicated GET /api/lessons/{lesson_id}/mastery/skills endpoint remains available for clients that only need skill coverage. A normalized per-skill snapshot is also available at `GET /api/lessons/{lesson_id}/mastery/skills/{skill}` for clients that need one skill without downloading the full collection; the path value is trimmed and case-folded before lookup. `GET /api/lessons/{lesson_id}/mastery/skills/next` now returns the highest-priority non-mastered skill using the same struggling → unseen → learning policy, with lower mastery coverage as the tie-break. The lesson-level GET /api/lessons/{lesson_id}/mastery response is the preferred lesson-screen snapshot: it carries both lesson and skill progress, so the lesson UI hydrates both views from one request and refreshes them together after an answer.
