...
const copy = {
  ar: {
    memory: 'الذاكرة', matching: 'المطابقة', ordering: 'الترتيب', reset: 'إعادة', moves: 'المحاولات', pairs: 'الأزواج',
    match: 'طابق العنصرين المتشابهين', chooseLeft: 'اختر كلمة', chooseRight: 'اختر ترجمتها',
    order: 'اضغط العناصر بالترتيب الصحيح', selected: 'المختار', complete: 'أحسنت! أكملت التحدي.',
    up: 'أعلى', down: 'أسفل', undo: 'تراجع', clear: 'مسح',
  },
  fr: {
    memory: 'Mémoire', matching: 'Association', ordering: 'Classement', reset: 'Réinitialiser', moves: 'Coups', pairs: 'Paires',
    match: 'Associe les deux éléments identiques', chooseLeft: 'Choisis un mot', chooseRight: 'Choisis sa traduction',
    order: 'Appuie sur les éléments dans le bon ordre', selected: 'Sélection', complete: 'Bravo ! Défi terminé.',
    up: 'Monter', down: 'Descendre', undo: 'Annuler', clear: 'Effacer',
  },
  en: {
    memory: 'Memory', matching: 'Matching', ordering: 'Ordering', reset: 'Reset', moves: 'Moves', pairs: 'Pairs',
    match: 'Match the two identical items', chooseLeft: 'Choose a word', chooseRight: 'Choose its translation',
    order: 'Tap the items in the correct order', selected: 'Selected', complete: 'Great job! Challenge complete.',
    up: 'Up', down: 'Down', undo: 'Undo', clear: 'Clear',
  },
} as const
...
const ORDER_ITEMS = {
  ar: ['الثاني', 'الرابع', 'الأول', 'الثالث'],
  fr: ['deux', 'quatre', 'un', 'trois'],
...
export function InteractiveGameBoard({ mode, lang, onComplete }: Props) {
  ...
  useEffect(() => {
    ...
    const correct = pairs[left]?.[1] === pairs[right]?.[1]
    const timer = window.setTimeout(() => {
      setMoves((value) => value + 1)
      if (correct && pairs.every((pair, index) => index === left || pair[1] !== pairs[left]?.[1])) {
        setCompleted(true)
        onComplete?.()
      }
      setLeft(null)
  ...
  }, [completed, left, right, mode, pairs, onComplete])
  ...
  function flipCard(index: number) {
    ...
    if (next[first].pair === next[index].pair) {
      setTimeout(() => {
        setMemoryCards((current) => current.map((item, itemIndex) =>
          itemIndex === first || itemIndex === index ? { ...item, matched: true } : item
        ))
        setFirst(null)
        setLocked(false)
      }, 250)
      ...
      setTimeout(() => {
        setMemoryCards((current) => current.map((item, itemIndex) =>
          itemIndex === first || itemIndex === index ? { ...item, flipped: false } : item
        ))
        setFirst(null)
        setLocked(false)
      }, 700)
  ...
  function chooseOrder(item: string) {
    ...
    if (next.length === targetOrder.length) {
      if (next.every((value, index) => value === targetOrder[index])) {
        setCompleted(true)
        onComplete?.()
      } else {
        setMoves((value) => value + 1)
  ...
  function shiftOrder(index: number, direction: -1 | 1) {
    setOrder((value) => {
      ...
      if (next.length === targetOrder.length && next.every((item, itemIndex) => item === targetOrder[itemIndex])) {
        setCompleted(true)
        onComplete?.()
      }
      return next
    ...
    <div className="interactive-game" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
          ...
          <div className="matching-board">
            <div>
              {pairs.map((pair, index) => (
                <button key={pair[0]} type="button" className={`match-option ${left === index ? 'selected' : ''}`} onClick={() => setLeft(index)}>{pair[0]}</button>
              ))}
            </div>
            <div>
              {pairs.map((pair, index) => (
                <button key={pair[1]} type="button" className={`match-option ${right === index ? 'selected' : ''}`} onClick={() => setRight(index)}>{pair[1]}</button>
              ))}
            </div>
          </div>
        </>
      ...
      {mode === 'ordering' && (
        <>
          ...
          <div className="ordering-pool">
            {ORDER_ITEMS[lang].map((item) => (
              <button key={item} type="button" disabled={order.includes(item)} onClick={() => chooseOrder(item)}>{item}</button>
            ))}
          </div>
          <div className="ordering-result">
            {order.map((item, index) => (
              <div key={item} className="order-row">
                ...
                <button type="button" onClick={() => shiftOrder(index, -1)} disabled={index === 0}>{t.up}</button>
                <button type="button" onClick={() => shiftOrder(index, 1)} disabled={index === order.length - 1}>{t.down}</button>
              </div>
            ))}
          </div>
