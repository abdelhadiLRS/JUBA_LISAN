...
export default function StatusIndicator({ status, userSpeaking, assistantSpeaking }: Props) {
  ...
  let label: string
  let dotClass = 'text-[rgba(32,33,39,.52)]'
  let pulse = false
  if (status === 'loading') label = t('statusLoading')
    ...
    if (userSpeaking) { label = t('statusDetecting'); dotClass = 'text-[#5862e2]'; pulse = true }
    else if (assistantSpeaking) { label = t('statusSpeaking'); dotClass = 'text-[#5862e2]'; pulse = true }
  ...
  return <div className="flex items-center gap-2 rounded-full border border-[rgba(7,7,9,.08)] bg-[#f3f7ef] px-3 py-1.5">
    <span className={`text-xs leading-none ${dotClass} ${pulse ? 'animate-pulse' : ''}`} aria-hidden="true">●</span>
    <span className="text-[rgba(32,33,39,.52)] text-[0.68rem] font-semibold tracking-[0.14em] uppercase">{label}</span>
  </div>
}