...
export default function LanguageSwitcher() {
  ...
  const skeleton = useMemo(() => (
    <div className="flex items-center gap-2 rounded-[14px] border border-[var(--juba-app-line)] bg-[var(--juba-app-surface)] px-3 py-2.5 shadow-[2px_2px_0_var(--juba-app-ink)] animate-pulse">
  ...
  return (
    <div ref={ref} className="relative w-full">
      ...
      {open && multiple && (
        <div className="absolute left-0 right-0 top-[calc(100%+8px)] z-50 overflow-hidden rounded-[18px] border-2 border-[var(--juba-app-ink)] bg-[var(--juba-app-surface)] p-1.5 shadow-[5px_5px_0_var(--juba-app-ink)]" role="listbox">
          ...
          {[...supportedUserLanguages].sort((a, b) => tTarget(a.target_language).localeCompare(tTarget(b.target_language))).map((ulang) => {
            ...
            return (
              <button
                ...
                className={`flex w-full items-center gap-3 rounded-[12px] px-3 py-2.5 text-left text-sm font-bold transition-all ${
                  ulang.is_active
                    ? 'bg-[var(--juba-app-green-soft)] text-[var(--juba-app-ink)]'
                    : 'text-[var(--juba-app-ink)] hover:bg-[var(--juba-app-green-soft)]'
                }`}
              >
                <Image src={lang.flagPath} alt="" width={22} height={15} className="shrink-0 rounded-sm object-cover" />
