    ...
    <>
      {parts.map((part, i) => {
        if (part.startsWith('**') && part.endsWith('**')) {
          return (
            <strong key={i} className="text-[#202127] font-bold">
              {part.slice(2, -2)}
            </strong>
          )
        }
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-4 p-6">
      <nav className="text-[#202127] text-[rgba(32,33,39,.52)] flex items-center gap-2 font-mono">
        ...
        <span>{'\u203a'}</span>
        <span className="text-[#202127] tracking-wide">{topic.title}</span>
      </nav>
                    ...
                    <div className="border-[#ededff] space-y-2 border-t pt-3">
                      ...
                      {nativeHelp.examples.map((ex, i) => (
                        <div key={i} className="space-y-0.5">
                          ...
                          <p className="text-[rgba(32,33,39,.52)] text-sm">{ex.note}</p>
                        </div>
                      ))}
                    </div>
                  )}
