      ...
      <div className="mx-auto max-w-7xl space-y-8">
        <header className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          ...
          <button
            ...
            disabled={refreshing}
            className="inline-flex items-center justify-center gap-2 rounded-xl border border-fl-border bg-fl-surface px-4 py-2.5 text-sm font-bold text-fl-fg transition hover:border-[var(--juba-primary)] disabled:opacity-50"
          >
          ...
          <div className="juba-card p-6 sm:p-8">
            ...
            <div className="space-y-3">
              {(plan.lessons ?? []).slice(0, 4).map((lesson, index) => (
                <Link key={`${lesson.id}-${index}`} href={lesson.id ? `/lesson/${lesson.id}` : '/plan'} className="group flex items-center gap-4 rounded-2xl border border-fl-border bg-fl-surface p-4 transition hover:-translate-y-0.5 hover:border-[var(--juba-primary)]">
                  <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${lesson.is_completed ? 'bg-[var(--juba-warm-soft)] text-[var(--juba-primary-dark)]' : 'bg-[var(--juba-primary-soft)] text-[var(--juba-primary-dark)]'}`}>
          ...
          <div className="juba-card p-6 sm:p-8">
            ...
            <div className="mt-5 grid grid-cols-2 gap-3">
              {scenarios.map((scenario) => (
                <Link key={scenario.title} href={scenario.href} className="rounded-2xl border border-fl-border p-4 transition hover:-translate-y-0.5 hover:border-[var(--juba-primary)] hover:bg-[var(--juba-primary-soft)]">
  ...
  return (
    <Link href={href} className="juba-card group p-5">
      <div className="flex items-center justify-between">
        <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[var(--juba-primary-soft)] text-[var(--juba-primary-dark)]">{icon}</span>
        <ArrowRight className="h-4 w-4 text-fl-muted-3 transition group-hover:translate-x-1" />
      </div>
