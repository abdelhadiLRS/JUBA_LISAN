    ...
    {!completed && exercises.length > 0 && <div className="min-w-[180px] rounded-[22px] bg-white/12 p-4 backdrop-blur-sm">
      ...
      <div className="mt-2 h-2 overflow-hidden rounded-full bg-white/20"><div className="h-full rounded-full bg-[var(--juba-yellow)] transition-all" style={{width:`${progress}%`}}/></div>
        ...
        <section className="juba-card rounded-[28px] p-6 sm:p-9">
          ...
          {contentItems.length > 0 && <div className="mb-9 grid gap-4 sm:grid-cols-2">{contentItems.slice(0, 4).map((item, i) => <article key={`${item.word}-${i}`} onMouseUp={() => handleTextSelection(item.example ?? '', lesson.cefr_level)} className="rounded-[20px] border border-[#ebe7f5] bg-[#ffffff] p-5"><p className="text-2xl font-bold">{item.word}</p>{item.translation && <p className="mt-2 font-semibold text-[#39751d]">{item.translation}</p>}{item.example && <p className="mt-4 text-sm font-medium text-[#242033]">“{item.example}”</p>}{item.word && <button type="button" onClick={() => { window.getSelection()?.removeAllRanges(); handleTextSelection(item.example ?? '', lesson.cefr_level); setTimeout(() => handleSaveWord(), 0) }} className="mt-4 rounded-[18px] border border-[#ebe7f5] bg-[#f2f0e8] px-4 py-2 text-xs font-bold text-[#242033] transition-colors hover:bg-[#e2f2d3]">{t('saveWord')}</button>}</article>)}</div>}
          {masteryReviewMode && <div className="mb-5 rounded-[26px] border border-[#5a9b32] bg-[#e2f2d3] p-4" role="status" aria-live="polite"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[.12em] text-[#777087]">{t('masteryReviewSession')}</p><p className="mt-1 text-sm font-semibold">{t('masteryReviewProgress', { count: masteryReviewCompleted })}</p>{masteryReviewSkill && <p className="mt-1 text-xs font-extrabold text-[#39751d]">{t('skillMastery')}: {masteryReviewSkill}</p>}{masteryNext && <p className="mt-1 text-xs font-medium text-[#777087]">{masteryNext.reason === 'struggling' ? t('masteryStruggling') : masteryNext.reason === 'unseen' ? t('masteryUnseen') : t('masteryReviewLowest')}</p>}</div><button type="button" onClick={finishMasteryReview} disabled={loadingMasteryNext} className="rounded-[18px] border border-[#ebe7f5] bg-[#f2f0e8] px-4 py-2 text-xs font-bold disabled:opacity-40">{t('masteryReviewFinish')}</button></div></div>}
          {masteryReviewImproved && <div className="mb-5 rounded-[26px] border border-[#ebe7f5] bg-[rgba(255,216,90,.30)] p-4" role="status" aria-live="polite"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-extrabold">{t('masteryReviewImproved')}</p><p className="mt-1 text-sm font-medium text-[#777087]">{t('masteryReviewImprovedDesc')}</p></div>{masteryReviewInitialRate !== null && masteryReviewFinalRate !== null && <span className="rounded-[18px] bg-[#f2f0e8] px-3 py-1 text-sm font-extrabold">Δ +{Math.max(0, Math.round((masteryReviewFinalRate - masteryReviewInitialRate) * 100))}%</span>}</div><button type="button" onClick={() => setMasteryReviewImproved(false)} className="mt-3 rounded-[18px] border border-[#ebe7f5] bg-[#f2f0e8] px-4 py-2 text-xs font-bold">{t('masteryReviewFinish')}</button></div>}
          ...
          {nextSkillMastery && <div className="mb-4 rounded-[26px] border border-[#ebe7f5] bg-[#e2f2d3] p-4" role="status" aria-live="polite"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[.12em] text-[#777087]">{t('skillMastery')}</p><p className="mt-1 font-extrabold">{nextSkillMastery.skill.skill}</p><p className="mt-1 text-xs font-medium text-[#777087]">{nextSkillMastery.reason === 'struggling' ? t('masteryStruggling') : nextSkillMastery.reason === 'unseen' ? t('masteryUnseen') : t('masteryReviewLowest')}</p></div><div className="flex items-center gap-3 text-right"><div><span className="rounded-[18px] bg-[#f2f0e8] px-3 py-1 text-xs font-bold">{t(`masteryStates.${nextSkillMastery.skill.mastery_state}`)}</span><p className="mt-1 text-sm font-extrabold">{Math.round(nextSkillMastery.skill.mastery_rate * 100)}%</p></div><button type="button" aria-label={`${t('skillMasteryPractice')}: ${nextSkillMastery.skill.skill}`} onClick={() => void startSkillMasteryReview(nextSkillMastery.skill.skill)} disabled={loadingMasteryNext} className="rounded-[20px] border border-[#ebe7f5] bg-[#f2f0e8] px-3 py-2 text-xs font-extrabold disabled:opacity-50">{t('skillMasteryPractice')}</button></div></div></div>}
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        active
                          ...
                          : answered
                            ? 'border-[#ebe7f5] bg-[#f2f0e8] text-[#39751d] hover:bg-[#e2f2d3]'
                            : 'border-[#ebe7f5] bg-[#f2f0e8] text-[#777087] hover:bg-[#e2f2d3]',
                      )}
                    >
                      {answered ? '✓' : index + 1}
                    </button>
          ...
          {!completed && exercise && <div className="rounded-[24px] border border-[#ebe7f5] bg-[#ffffff] p-6 sm:p-8"><div className="flex items-center justify-between gap-4"><span className="rounded-[18px] bg-[#e2f2d3] px-3 py-1 text-xs font-bold text-[#39751d]">{t('exerciseProgress', { current: currentExercise + 1, total: exercises.length })}</span><span className="text-xs font-bold text-[#777087]">{variantLabel}</span></div><div className="mt-6 flex flex-wrap gap-2" aria-label={variantLabel}>
              <span className="rounded-[18px] border border-[#ebe7f5] bg-[#f2f0e8] px-3 py-1 text-[11px] font-extrabold text-[#777087]">{variantLabel}</span>
            </div>
            <h2 className="mt-5 text-2xl font-bold sm:text-3xl">{exercise.question}</h2>
            {isListeningExercise && <Link href="/listening" className="mt-5 inline-flex items-center rounded-[18px] border border-[#ebe7f5] bg-[#f2f0e8] px-4 py-2 text-sm font-bold text-[#242033] transition-colors hover:border-[#5a9b32] hover:bg-[#e2f2d3]">{t('listeningPractice')}</Link>}
            {(exerciseAudioUrl || exerciseTranscript) && <div className="mt-5 rounded-[26px] border border-[#ebe7f5] bg-[#f2f0e8] p-4" role="region" aria-label={t('listen')}>
            ...
            {isChoiceExercise && <div className="mt-6 grid gap-3 sm:grid-cols-2" role="group" aria-label={variantLabel}>{exercise.options?.map((option, optionIndex) => <button type="button" key={option} aria-pressed={answer === option} onClick={() => setAnswer(option)} className={cn('group flex min-h-16 items-center gap-3 rounded-[26px] border border-[#ebe7f5] bg-[#f2f0e8] p-4 text-left font-semibold text-[#242033] transition-all hover:-translate-y-0.5 hover:border-[#5a9b32] hover:bg-[#e2f2d3]', answer === option && 'border-[var(--juba-violet-dark)] bg-[#e2f2d3] shadow-sm')}><span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-[18px] border border-[#ebe7f5] text-xs font-extrabold">{String.fromCharCode(65 + optionIndex)}</span><span className="flex-1">{option}</span>{answer === option && <span aria-hidden="true" className="font-extrabold">✓</span>}</button>)}</div>}
            {isLongFormExercise && <textarea value={answer} onChange={(e) => setAnswer(e.target.value)} className="mt-6 min-h-36 w-full resize-y rounded-[26px] border border-[#ebe7f5] bg-[#f2f0e8] p-4 font-semibold leading-7 text-[#242033] outline-none transition-shadow focus:ring-2 focus:ring-[#5a9b32]" placeholder={t('typeAnswer')} aria-label={t('typeAnswer')} disabled={evaluating || !!exercise.feedback} />}
            {isShortAnswerExercise && <input value={answer} onChange={(e) => setAnswer(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') void submitAnswer() }} className="mt-6 w-full rounded-[26px] border border-[#ebe7f5] bg-[#f2f0e8] p-4 font-semibold text-[#242033] outline-none transition-shadow focus:ring-2 focus:ring-[#5a9b32]" placeholder={t('typeAnswer')} aria-label={t('typeAnswer')} disabled={evaluating || !!exercise.feedback} />}{exercise.feedback && <div className="mt-5 rounded-[26px] border border-[#ebe7f5] bg-[rgba(255,216,90,.30)] p-4 font-semibold text-[#242033]" role="status" aria-live="polite">{exercise.feedback}</div>}<div className="mt-5 flex flex-wrap gap-3"><button type="button" onClick={() => void loadNativeHint()} disabled={loadingHint} className="rounded-[18px] border border-[#ebe7f5] bg-[#f2f0e8] px-4 py-2 text-sm font-bold text-[#242033] transition-colors hover:bg-[#e2f2d3] disabled:opacity-40">{loadingHint ? t('loadingHint') : t('hint')}</button><button type="button" onClick={() => void loadNativeExplanation()} disabled={loadingExplanation} className="rounded-[18px] border border-[#ebe7f5] bg-[#f2f0e8] px-4 py-2 text-sm font-bold text-[#242033] transition-colors hover:bg-[#e2f2d3] disabled:opacity-40">{loadingExplanation ? t('loadingExplanation') : t('explain')}</button><button type="button" aria-expanded={attemptsOpen} onClick={() => setAttemptsOpen((value) => !value)} className="rounded-[18px] border border-[#ebe7f5] bg-[#f2f0e8] px-4 py-2 text-sm font-bold text-[#242033] transition-colors hover:bg-[#e2f2d3]">{attemptsOpen ? t('hideAttempts') : t('attempts', { count: attempts.length })}</button></div>{nativeHint && <div className="mt-4 rounded-[26px] border border-[#ebe7f5] bg-[#e2f2d3] p-4 font-semibold text-[#242033]" role="status" aria-live="polite"><span className="font-extrabold">{t('hintPrefix')}</span>{nativeHint}</div>}{nativeExplanation && <div className="mt-4 rounded-[26px] border border-[#ebe7f5] bg-[#f2f0e8] p-4 font-medium text-[#242033]" role="status" aria-live="polite"><p className="font-extrabold">{t('explanation')}</p><p className="mt-2 whitespace-pre-wrap">{nativeExplanation}</p></div>}{attemptsOpen && <div className="mt-4 overflow-hidden rounded-[26px] border border-[#ebe7f5]"><div className="bg-[#f2f0e8] px-4 py-3 text-sm font-extrabold">{t('attemptHistory')}</div><div className="grid grid-cols-3 gap-2 border-b border-[#ebe7f5] p-3 text-center text-xs"><div><p className="text-[#777087]">{t('attemptsLabel')}</p><p className="mt-1 text-base font-extrabold">{attempts.length}</p></div><div><p className="text-[#777087]">{t('best')}</p><p className="mt-1 text-base font-extrabold">{attempts.length ? Math.round(Math.max(...attempts.map((item) => item.score)) * 100) : 0}%</p></div><div><p className="text-[#777087]">{t('latest')}</p><p className="mt-1 text-base font-extrabold">{attempts.length ? Math.round(attempts[attempts.length - 1].score * 100) : 0}%</p></div></div>{attempts.length === 0 ? <p className="p-4 text-sm font-medium text-[#777087]">{t('noAttempts')}</p> : <div className="divide-y divide-[#ebe7f5]">{attempts.map((item) => <div key={item.id} className="flex flex-wrap items-center justify-between gap-3 p-4 text-sm"><div><span className="font-extrabold">#{item.attempt_number}</span><span className="ml-2 text-[#777087]">{(item.variant ?? t('exercise')).replace(/[_-]+/g, ' ')}</span></div><span className="font-extrabold">{Math.round(item.score * 100)}%</span></div>)}</div>}</div>}<div className="mt-6 flex flex-wrap gap-3">{!masteryReviewMode && currentExercise > 0 && <button type="button" onClick={() => setCurrentExercise((value) => Math.max(0, value - 1))} disabled={evaluating} aria-label={t('backToPlan')} className="rounded-[18px] border border-[#ebe7f5] bg-[#f2f0e8] px-6 py-3 font-bold text-[#242033] transition-colors hover:bg-[#e2f2d3] disabled:opacity-40">←</button>}{exercise.feedback && hasAdaptiveTarget(exercise) && isAdaptiveRetryAction(exercise.recommended_action) && <button type="button" onClick={() => void adaptiveNextExercise()} disabled={evaluating} className="rounded-[18px] border border-[#ebe7f5] bg-[#f2f0e8] px-6 py-3 font-bold text-[#242033] transition-colors hover:bg-[#e2f2d3] disabled:opacity-40">{normaliseAdaptiveAction(exercise.recommended_action) === 'advance_harder' ? t('retryHarderVariant') : t('retryEasierVariant')}</button>}{exercise.feedback && isAdaptiveAdvanceAction(exercise.recommended_action) && <button type="button" onClick={() => masteryReviewMode ? void continueMasteryReview() : currentExercise + 1 < exercises.length ? setCurrentExercise(currentExercise + 1) : void finishLesson()} disabled={evaluating} className="rounded-[18px] bg-[#e2f2d3] px-6 py-3 font-bold text-[#242033] transition-colors hover:opacity-90 disabled:opacity-40">{t('advance')}</button>}{exercise.feedback && exercise.score !== null && exercise.score < 0.5 && !hasAdaptiveTarget(exercise) && !isAdaptiveReinforcementAction(exercise.recommended_action) && <button type="button" onClick={() => void retryExercise()} disabled={evaluating} className="rounded-[18px] border border-[#ebe7f5] bg-[#f2f0e8] px-6 py-3 font-bold text-[#242033] transition-colors hover:bg-[#e2f2d3] disabled:opacity-40">{t('retryEasier')}</button>}<button type="button" aria-keyshortcuts="Enter" onClick={() => void submitAnswer()} disabled={!answer.trim() || evaluating || !!exercise.feedback} className="rounded-[18px] bg-[#39751d] px-6 py-3 font-bold text-white transition-transform hover:-translate-y-0.5 disabled:opacity-40">{evaluating ? t('checking') : t('checkAnswer')}</button>{exercise.feedback && <button type="button" aria-keyshortcuts="Enter" onClick={() => masteryReviewMode ? void continueMasteryReview() : currentExercise + 1 < exercises.length ? setCurrentExercise(currentExercise + 1) : void finishLesson()} disabled={evaluating || loadingMasteryNext} className="rounded-[18px] border border-[#ebe7f5] bg-[#f2f0e8] px-6 py-3 font-bold text-[#242033] transition-colors hover:bg-[#e2f2d3] disabled:opacity-40">{masteryReviewMode ? (loadingMasteryNext ? t('masteryReviewLoading') : t('masteryReviewNext')) : currentExercise + 1 < exercises.length ? t('next') : t('finish')}</button>}</div></div>}
          {masteryReviewExhausted && !masteryReviewMode && !masteryReviewImproved && <div className="mb-5 rounded-[26px] border border-[#ebe7f5] bg-[#e2f2d3] p-5" role="status" aria-live="polite"><p className="font-extrabold">{masteryReviewSkill ? t('skillMasteryExhausted') : t('masteryReviewExhausted')}</p><p className="mt-1 text-sm font-medium text-[#777087]">{masteryReviewSkill ? t('skillMasteryExhaustedDesc', { skill: masteryReviewSkill }) : t('masteryReviewExhaustedDesc')}</p><button type="button" onClick={finishMasteryReview} className="mt-4 rounded-[18px] border border-[#ebe7f5] bg-[#f2f0e8] px-5 py-2 font-bold text-[#242033] transition-colors hover:bg-[#e2f2d3]">{t('masteryReviewFinish')}</button></div>}
          {completed && <div className="rounded-[24px] border border-[#ebe7f5] bg-[rgba(255,216,90,.30)] p-8 text-center">
            ...
            {attemptStats && <div className="mx-auto mt-6 grid max-w-xl grid-cols-3 gap-3">
              <div className="rounded-[26px] border border-[#ebe7f5] bg-[#f2f0e8] p-4"><p className="text-xs font-bold text-[#777087]">{t('attemptsLabel')}</p><p className="mt-1 text-2xl font-extrabold">{attemptStats.totalAttempts}</p></div>
