...
interface ExerciseItem { id: number; exercise_type: string; question: string; options: string[] | null; correct_answer: string; explanation: string | null; native_explanation: string | null; user_answer: string | null; score: number | null; feedback: string | null; native_hint: string | null; content_id?: string | null; variant?: string | null; accepted_answers?: string[] | null; metadata?: Record<string, string> | null; recommended_action?: string; recommended_variant?: string | null; mastery_score?: number; mastery_state?: string; mastery_variants?: number }
...
interface LessonMasterySnapshot { total_exercises: number; attempted_exercises: number; mastered_exercises: number; learning_exercises: number; struggling_exercises: number; unseen_exercises: number; average_mastery_score: number; covered_variants: number }
...
export default function LessonPage() {
  ...
  useEffect(() => {
    ...
    apiFetch(`/api/lessons/${id}`).then(async (res) => { if (!res.ok) throw new Error('lesson_fetch_failed'); return res.json() }).then((data: { lesson: LessonData; exercises: ExerciseItem[] }) => {
      if (!cancelled) { setLesson(data.lesson); setExercises(data.exercises || []); setCompleted(!!data.lesson.is_completed); void loadAttemptSummary(data.lesson.id); void loadLessonMastery(data.lesson.id) }
