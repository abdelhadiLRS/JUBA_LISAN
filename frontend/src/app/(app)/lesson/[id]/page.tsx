...
export default function LessonPage() {
  ...
  const difficulty: Record<string, number> = { multiple_choice: 1, fill_blank: 2, translate: 3, listening: 3, pronunciation: 3, free_write: 4 }
  const currentVariant = exercise?.variant ?? exercise?.exercise_type ?? ''
  const easierRetryIndex = useMemo(() => {
    if (!exercise || exercise.score === null || exercise.score >= 1 || !exercise.content_id) return null
    const currentLevel = difficulty[currentVariant] ?? 0
    let best: { index: number; level: number } | null = null
    exercises.forEach((item, index) => {
      if (index === currentExercise || item.content_id !== exercise.content_id || item.feedback) return
      const level = difficulty[item.variant ?? item.exercise_type] ?? 0
      if (level > 0 && level < currentLevel && (!best || level > best.level)) best = { index, level }
    })
    return best?.index ?? null
  }, [exercise, currentExercise, exercises, currentVariant])
