...
export default function LessonPage() {
  ...
  useEffect(() => { getGrammarTopics(activeLanguage?.code ?? 'en-GB').then(() => undefined).catch(() => undefined) }, [activeLanguage?.code])
  useEffect(() => { fetchFreemium().catch(() => undefined) }, [fetchFreemium])
  useEffect(() => {
    let cancelled = false
    apiFetch(`/api/lessons/${id}`).then(async (res) => { if (!res.ok) throw new Error('lesson_fetch_failed'); return res.json() }).then((data: { lesson: LessonData; exercises: ExerciseItem[] }) => { if (!cancelled) { setLesson(data.lesson); setExercises(data.exercises || []); setCompleted(!!data.lesson.is_completed) } }).catch(() => { if (!cancelled) router.replace('/plan') })
  ...
  const finishLesson = useCallback(async () => { if (!lesson || completed) return; setEvaluating(true); try { const res = await apiFetch(`/api/lessons/${lesson.id}/complete`, { method: 'POST' }); if (!res.ok) throw new Error('complete_failed'); const updated: LessonData = await res.json(); setLesson(updated); setCompleted(true); completeLesson(lesson.id); setDayComplete(true) } catch { /* keep lesson active so the user can retry */ } finally { setEvaluating(false) } }, [lesson, completed, completeLesson])
  const submitAnswer = async () => {
    if (!exercise || !answer.trim() || evaluating || exercise.feedback) return; setEvaluating(true)
    try { const res = await apiFetch(`/api/lessons/exercises/${exercise.id}/answer`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ answer: answer.trim() }) }); if (!res.ok) throw new Error('answer_failed'); const result = await res.json(); setExercises((prev) => prev.map((item) => item.id === exercise.id ? { ...item, ...result } : item)); setAnswer('') } catch { /* keep answer so the user can retry */ } finally { setEvaluating(false) }
  }
  ...
  return (
    <main className="min-h-screen bg-[var(--juba-paper,#f7f4ea)] px-4 py-5 sm:px-7 lg:px-10">
      <div className="mx-auto max-w-5xl">
        <header className="sticky top-0 z-20 mb-5 rounded-[24px] border-2 border-neutral-950 bg-white/95 p-4 shadow-[5px_5px_0_rgba(17,17,17,.9)] backdrop-blur dark:bg-neutral-900/95"><div className="flex items-center gap-4"><Link href="/courses" className="rounded-full border-2 border-neutral-950 px-3 py-2 text-sm font-black">←</Link><div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-3"><p className="truncate text-xs font-black uppercase tracking-[.18em] text-neutral-500">{lesson.cefr_level} · {lesson.lesson_type}</p><span className="text-sm font-black">{progress}%</span></div><div className="mt-2 h-3 overflow-hidden rounded-full border-2 border-neutral-950 bg-white"><div className="h-full rounded-full bg-[#d8f53f] transition-all" style={{ width: `${progress}%` }} /></div></div></div></header>
        {freemiumExhausted && <div className="mb-5"><FreemiumQuotaBanner /></div>}{dayComplete && <div className="mb-5 rounded-[24px] border-2 border-neutral-950 bg-[#d8f53f] p-4 font-black shadow-[4px_4px_0_rgba(17,17,17,.9)]">Daily goal complete — keep your streak alive.</div>}
