  ...
  return (
    <main className="juba-mobile-courses min-h-screen px-4 py-8 sm:px-6 lg:px-10">
      <div className="mx-auto max-w-6xl space-y-8">
        <section className="juba-card relative overflow-hidden rounded-[32px] border border-[rgba(7,7,9,.08)] bg-[#5862e2] p-7 text-white shadow-[0_12px_30px_rgba(43,45,90,.055)] sm:p-10">
          <div className="relative z-10 max-w-3xl">
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              <Link href="/courses" className="inline-flex items-center gap-2 rounded-[18px] bg-white px-5 py-3 font-black text-[#373fb8] shadow-[0_8px_18px_rgba(43,45,90,.10)] transition hover:-translate-y-0.5">{t('openRoadmap')} <ArrowRight className="h-4 w-4" /></Link>
        ...
        <section className="grid gap-4 md:grid-cols-3">
          {skills.map((key, index) => {
            ...
            return (
              <div key={key} className="juba-card rounded-[27px] border border-[rgba(7,7,9,.08)] bg-white p-5 shadow-[0_12px_30px_rgba(43,45,90,.055)]">
        ...
        <section>
          <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
            <div><p className="juba-eyebrow">{t('roadmapEyebrow')}</p><h2 className="mt-2 text-3xl font-black text-[#202127] sm:text-4xl">{t('roadmapTitle')}</h2></div>
          ...
          {loading ? (
            <div className="grid gap-5 lg:grid-cols-2" aria-label={t('loading')}>{CEFR_LEVELS.map((level) => <div key={level} className="juba-card h-64 animate-pulse rounded-[28px] border border-[rgba(7,7,9,.08)] bg-white p-6" />)}</div>
          ) : (
            <div className="grid gap-5 lg:grid-cols-2">
              {CEFR_LEVELS.map((level, index) => {
                ...
                return (
                  <article key={level} className={`juba-card relative rounded-[30px] border border-[rgba(7,7,9,.08)] bg-white p-6 shadow-[0_12px_30px_rgba(43,45,90,.055)] transition hover:-translate-y-1 ${current ? 'ring-2 ring-[#5862e2]' : ''}`}>
                    ...
                    {unlocked ? (
                      <Link href={current ? '/plan' : `/courses/${level}`} className="mt-6 inline-flex items-center gap-2 rounded-xl bg-[#ededff] px-5 py-3 font-black text-[#373fb8] transition hover:bg-[#5862e2]">{current ? t('openLearningPlan') : t('exploreLevel')} <ArrowRight className="h-4 w-4" /></Link>
                    ) : (
                      <span className="mt-6 inline-flex items-center gap-2 rounded-xl bg-[#f4f4f2] px-5 py-3 font-bold text-[rgba(32,33,39,.52)]"><LockKeyhole className="h-4 w-4" /> {t('unlockLater')}</span>
                    )}
                  </article>
                )
              })}
        ...
        <section className="juba-card rounded-[30px] border border-[rgba(7,7,9,.08)] bg-white p-6 shadow-[0_12px_30px_rgba(43,45,90,.055)] sm:p-7">
        ...
        <section className="juba-card rounded-[30px] border border-[rgba(7,7,9,.08)] bg-white p-6 shadow-[0_12px_30px_rgba(43,45,90,.055)] sm:p-7">
          <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="juba-eyebrow">{t('missionsEyebrow')}</p><h2 className="mt-2 text-2xl font-black text-[#202127]">{t('missionsTitle')}</h2></div><Link href="/learning-journey" className="font-bold text-[#373fb8] underline underline-offset-4">{t('openRoadmap')}</Link></div>
        ...
        <section className="juba-card rounded-[30px] border border-[rgba(7,7,9,.08)] bg-white p-6 shadow-[0_12px_30px_rgba(43,45,90,.055)] sm:p-7">
          <div className="flex items-center gap-3"><Sparkles className="h-6 w-6 text-[#373fb8]" /><h2 className="text-2xl font-black text-[#202127]">{t('rhythmTitle')}</h2></div>
