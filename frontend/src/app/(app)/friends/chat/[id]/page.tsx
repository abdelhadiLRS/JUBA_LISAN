...
export default function FriendChatPage() {
  ...
  return <div className="mx-auto flex h-[calc(100dvh-110px)] max-w-5xl flex-col px-4 py-5 sm:px-6">
    ...
    <div className="juba-panel flex min-h-0 flex-1 flex-col p-0">
      ...
      <div className="border-t border-[var(--juba-border)] p-3 sm:p-4"><div className="flex gap-2"><input disabled={!friend||!!error}  value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==='Enter'&&!e.shiftKey&&send()} placeholder="Write a message…" className="juba-input flex-1"/><button onClick={send} disabled={!input.trim()||sending} className="juba-primary-button"><Send className="h-4 w-4"/>{sending?'Sending':'Send'}</button></div></div>
    </div>
  </div>
}
