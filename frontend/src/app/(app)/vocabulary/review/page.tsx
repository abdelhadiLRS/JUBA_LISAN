...
export default function VocabularyReviewPage() {
  ...
  if (error && !words.length) return <main className="mx-auto max-w-4xl p-6 sm:p-10"><div className="juba-card p-10 text-center" aria-live="polite"><h1 className="text-2xl font-black">{t('unavailable')}</h1><p className="mt-3 text-fl-muted-1">{error}</p><button type="button" onClick={() => void loadReviewCards()} disabled={loading} className="mt-6 rounded-full border-2 border-fl-border bg-fl-accent px-6 py-3 font-black disabled:opacity-40">{t('tryAgain')}</button></div></main>
  if (!words.length) return <main className="mx-auto max-w-3xl p-6"><div className="juba-card p-8 text-center"><h1 className="mt-3 text-3xl font-black">{t('noDue')}</h1><p className="mt-3 text-fl-muted-1">{authenticated ? t('caughtUp') : t('guestHint')}</p><Link href={authenticated ? '/vocabulary' : '/translator'} className="mt-6 inline-block rounded-full border-2 border-fl-border bg-fl-accent px-6 py-3 font-black">{authenticated ? `${t('openVocabulary')} →` : `${t('openTranslator')} →`}</Link></div></main>

  const finished = done >= words.length
  return <main className="mx-auto max-w-4xl p-6 sm:p-10"><div className="mb-6 flex items-center justify-between gap-4"><div><h1 className="mt-2 text-3xl font-black sm:text-4xl">{t('title')}</h1><p className="mt-1 text-sm font-bold text-fl-muted-1">{authenticated ? `${words.length} ${t('cardsDue')}` : `${remainingDue} ${t('dueNow')} · ${words.length} ${t('saved')}`}</p></div><Link href="/vocabulary" className="font-bold underline">{t('vocabulary')}</Link></div><div className="mb-6 h-2 overflow-hidden rounded-full bg-fl-surface-2" role="progressbar" aria-valuemin={0} aria-valuemax={100} aria-valuenow={progress} aria-label={t('progressLabel')}><div className="h-full bg-fl-accent transition-all" style={{ width: `${progress}%` }} /></div>{error && <div className="mb-4 rounded-xl border-2 border-fl-border p-3 text-sm font-bold" role="alert">{error}</div>}
    {finished ? <div className="juba-card p-10 text-center"><div className="text-5xl">✓</div><h2 className="mt-4 text-3xl font-black">{t('complete')}</h2><p className="mt-2 text-fl-muted-1">{t('reviewed', { count: words.length })}</p><button type="button" onClick={restart} className="mt-6 rounded-full border-2 border-fl-border bg-fl-accent px-6 py-3 font-black">{t('reviewAgain')}</button></div> : <div className="juba-card overflow-hidden p-6 sm:p-10"><div className="flex items-center justify-between text-sm font-bold text-fl-muted-1"><span>{index + 1} / {words.length}</span><span>{authenticated ? t('account') : `${current.source || t('source')} → ${current.target || t('target')}`}</span></div><button type="button" onClick={speak} className="mx-auto mt-10 block text-5xl font-black sm:text-7xl" dir="auto">{current.word}</button><p className="mt-3 text-center text-sm font-bold text-fl-muted-1">{t('tapToHear')}</p><p className="mt-1 text-center text-xs font-medium text-fl-muted-1" aria-label={t('shortcutHint')}>{revealed ? t('rateKeys') : t('spaceShortcut')}</p>{revealed && <div className="mt-10 border-2 border-fl-border bg-fl-surface-soft p-8 text-center"><p className="text-3xl font-black" dir="auto">{current.translation || current.definition}</p>{current.example_sentence && <p className="mt-3 text-fl-muted-1" dir="auto">{current.example_sentence}</p>}</div>}<div className="mt-8 text-center"><button type="button" aria-label={t('reveal')} aria-expanded={revealed} onClick={() => setRevealed(true)} disabled={revealed} className="rounded-full border-2 border-fl-border bg-fl-accent px-8 py-3 font-black disabled:opacity-40">{t('reveal')}</button></div>{revealed && <div className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-4"><button type="button" aria-label={t('again')} onClick={() => review('again')} className="rounded-xl border-2 border-fl-border px-4 py-3 font-black disabled:opacity-40" disabled={reviewing}>{t('again')}</button><button type="button" aria-label={t('hard')} onClick={() => review('hard')} className="rounded-xl border-2 border-fl-border px-4 py-3 font-black disabled:opacity-40" disabled={reviewing}>{t('hard')}</button><button type="button" aria-label={t('good')} onClick={() => review('good')} className="rounded-xl border-2 border-fl-border bg-fl-accent px-4 py-3 font-black disabled:opacity-40" disabled={reviewing}>{t('good')}</button><button type="button" aria-label={t('easy')} onClick={() => review('easy')} className="rounded-xl border-2 border-fl-border px-4 py-3 font-black disabled:opacity-40" disabled={reviewing}>{t('easy')}</button></div>}</div>}
  </main>
}
