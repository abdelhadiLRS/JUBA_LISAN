    ...
    <main className="min-h-screen overflow-hidden bg-[var(--juba-bg)] text-[var(--juba-text)]">
      <section className="relative border-b-2 border-[var(--juba-violet)] bg-[var(--juba-violet)] text-white">
        <div className="mx-auto max-w-6xl px-5 pb-16 pt-8 sm:px-8 lg:pb-24 lg:pt-12">
          <nav className="mb-16 flex items-center justify-between">
            ...
            <Link href="/dashboard" className="rounded-full border-2 border-neutral-950 bg-[var(--juba-surface)] px-5 py-2.5 text-sm font-black shadow-[var(--juba-shadow-sm)]">My learning</Link>
          </nav>
          <div className="max-w-4xl">
            ...
            <div className="mt-9 flex flex-col gap-3 sm:flex-row">
              <Link href="/courses" className="inline-flex items-center justify-center gap-2 rounded-full border-2 border-neutral-950 bg-[var(--juba-text)] px-7 py-4 font-black text-white shadow-[var(--juba-shadow-sm)]">Start a lesson <ArrowRight className="h-5 w-5" /></Link>
              <Link href="/review" className="inline-flex items-center justify-center gap-2 rounded-full border-2 border-neutral-950 bg-white px-7 py-4 font-black shadow-[var(--juba-shadow-sm)]">Review vocabulary <Brain className="h-5 w-5" /></Link>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-5 py-16 sm:px-8 lg:py-24">
        <div className="mb-10 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          <div><p className="text-xs font-black uppercase tracking-[.2em] text-[var(--juba-muted)]">The loop</p><h2 className="mt-2 text-4xl font-black tracking-tight sm:text-5xl">Four steps. One habit.</h2></div>
        ...
        <div className="grid gap-5 md:grid-cols-2">
          {steps.map(({ n, title, text, icon: Icon }) => (
            <article key={n} className="rounded-[28px] border-2 border-neutral-950 bg-white p-7 shadow-[var(--juba-shadow-sm)] transition-transform hover:-translate-y-1">
              <div className="flex items-start justify-between"><span className="text-sm font-black text-[var(--juba-muted)]">{n}</span><div className="rounded-[24px] border-2 border-neutral-950 bg-[#d8f53f] p-3"><Icon className="h-5 w-5" /></div></div>
              <h3 className="mt-10 text-2xl font-black">{title}</h3><p className="mt-2 max-w-md font-semibold leading-relaxed text-[var(--juba-muted)]">{text}</p>
      ...
      <section className="mx-auto max-w-6xl px-5 py-16 sm:px-8 lg:py-24">
        <div className="rounded-[32px] border-2 border-neutral-950 bg-[#d8f53f] p-7 shadow-[var(--juba-shadow-md)] sm:p-10">
