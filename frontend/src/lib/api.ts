...
export function clearGuestCookie(): void {
  if (typeof document !== 'undefined') document.cookie = `${GUEST_COOKIE}=; Max-Age=0; Path=/; SameSite=Lax`
}
...
async function refreshToken(): Promise<string | null> {
  ...
  refreshPromise = (async () => {
    try {
      const res = await fetchAuthRequest(`${BASE_URL}/api/auth/refresh`, { method: 'POST', credentials: 'include' })
...
async function _apiFetch(url: string, options: RequestInit = {}): Promise<Response> {
  ...
  if (res.status === 401 && token && !isAuthEntryPoint) {
    ...
    if (newToken) { headers.set('Authorization', `Bearer ${newToken}`); res = await fetch(`${BASE_URL}${url}`, { ...requestOptions, headers }) }
  }
  return res
}
...
export function getGuestMemory(): TranslatorSavedWord[] {
  ...
  try {
    ...
    return Array.isArray(value) ? value.filter((x): x is TranslatorSavedWord => !!x && typeof x === 'object' && typeof x.word === 'string' && typeof x.translation === 'string') : []
  } catch { return [] }
}
