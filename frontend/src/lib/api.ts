...
export function ensureGuestCookie(): string | null {
  ...
  if (existing) {
    try { return decodeURIComponent(existing) } catch {}
  }
...
async function fetchAuthRequest(input: RequestInfo | URL, options: RequestInit): Promise<Response> {
  ...
  const controller = new AbortController(); const timeout = setTimeout(() => controller.abort(), AUTH_REQUEST_TIMEOUT_MS)
  try { return await fetch(input, { ...options, signal: controller.signal }) } finally { clearTimeout(timeout) }
}
async function refreshToken(): Promise<string | null> {
  if (isRefreshing && refreshPromise) return refreshPromise
  isRefreshing = true
  refreshPromise = (async () => {
    try { const res = await fetchAuthRequest(`${BASE_URL}/api/auth/refresh`, { method: 'POST', credentials: 'include' }); if (!res.ok) throw new Error('refresh failed'); const data = (await res.json()) as { access_token?: string }; if (!data.access_token) throw new Error('missing access token'); useAuthStore.getState().setTokens(data.access_token); return data.access_token }
...
async function _apiFetch(url: string, options: RequestInit = {}): Promise<Response> {
  const token = useAuthStore.getState().accessToken; const headers = new Headers(options.headers); headers.set('Accept', 'application/json'); if (token) headers.set('Authorization', `Bearer ${token}`)
  const requestOptions = { ...options, headers, credentials: 'include' as const, cache: 'no-store' as const }; const shouldTimeout = url === '/api/auth/me' || url === '/api/auth/refresh'
  let res = shouldTimeout ? await fetchAuthRequest(`${BASE_URL}${url}`, requestOptions) : await fetch(`${BASE_URL}${url}`, requestOptions)
  const isAuthEntryPoint = url === '/api/auth/login' || url === '/api/auth/register'
  if (res.status === 401 && token && !isAuthEntryPoint) { const newToken = await refreshToken(); if (newToken) { headers.set('Authorization', `Bearer ${newToken}`); res = await fetch(`${BASE_URL}${url}`, { ...requestOptions, headers }) } }
  return res
}
...
function normalizeGuestReviewCard(value: unknown): GuestReviewCard | null { if (!value || typeof value !== 'object' || Array.isArray(value)) return null; const item = value as Record<string, unknown>; if (!isFiniteNumber(item.repetitions) || !isFiniteNumber(item.interval) || !isFiniteNumber(item.ease) || !isFiniteNumber(item.due)) return null; return { repetitions: Math.max(0, item.repetitions), interval: Math.max(0, item.interval), ease: Math.max(1, item.ease), due: Math.max(0, item.due) } }
function normalizeGuestSavedWord(value: unknown): TranslatorSavedWord | null { if (!value || typeof value !== 'object' || Array.isArray(value)) return null; const item = value as Record<string, unknown>; if (typeof item.word !== 'string' || !item.word.trim() || typeof item.translation !== 'string' || !item.translation.trim() || typeof item.target !== 'string' || !item.target.trim()) return null; const normalized: TranslatorSavedWord = { source: typeof item.source === 'string' ? item.source : '', target: item.target, word: item.word, translation: item.translation }; if (typeof item.createdAt === 'string') normalized.createdAt = item.createdAt; return normalized }
