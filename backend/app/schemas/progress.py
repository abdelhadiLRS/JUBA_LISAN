...
class GameSessionStart(BaseModel):
    ...
    def validate_game_id(cls, value: str) -> str:
        value = value.strip()
        if value not in {"math", "words", "quick_choice", "context_quest", "listen_choose", "listening_detective", "spelling", "word_scramble", "fill_blank", "sequence", "memory", "matching", "ordering", "sentence_builder"}:
